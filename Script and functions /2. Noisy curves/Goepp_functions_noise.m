%% Simulated examples with NOISE (from Geopp 2025 sez 3.1)
clc; close all; clear;
...
switch test_case
    case 1 % bump function
        ...
        rng(seed)
        for i = 1:N 
            f(i) = 0.4*(x(i)+2*exp(-(16*(x(i)-0.5))^2));  % True data
            epsilon = ((0.3*x(i)+0.2*sqrt(x(i)))^2)*randn(1);  % Noisy
            y(i) = f(i) + epsilon;   % Noisy data
        end
    ...
    case 3 % sine function
        ...
        rng(seed)
        for i = 1:N
            f(i) = 0.5*sin(6*pi*x(i))+0.5;  % True data
            epsilon = 0.15*randn(1);   % Noise
            y(i) = f(i) + epsilon;     % Noisy data
        end
    ...
    case 4 % spahet function
        ...
        for i = 1:N
            ...
            f(i) = sqrt(x(i)*(1-x(i)))*sin(arg)+0.5;    % True data
            epsilon = ((0.3*x(i)+0.2*sqrt(x(i)))^2)*randn(1);   % Noise
            y(i) = f(i) + epsilon;  % Noisy data
        end
...
cvx_begin
    ...
    % Minimize the norm of Ac-y (in this case this is a vector so we
    % use the 2 norm) + lambda * norm 1 of Dc
    minimize( norm(A*c - y, 2) + lambda * norm(D*c, 1))
cvx_end
...
y_knots = -0.3*ones(size(active_knots)); 
% Plot
figure(2); 
hold on;
grid on;
...
plot(x,y,'ro', 'MarkerFaceColor','yellow');
% Plot the active knots as blue triangles
plot(active_knots, y_knots, 'b^', 'MarkerSize', 8,'MarkerFaceColor', 'b');
legend('True function','Reconstructed spline','Noisy data','Active knots');
...
active_knots_adj = AdjustKnotsPosition(delta_cluster,tol_adj,active_knots,p,x,y, a, b);
...
y_knots_adj = -0.3*ones(size(active_knots_adj)); 
...
legend('True function','Reconstructed spline','Noisy data', 'Final knots');
title(sprintf('Final B-spline reconstruction (p = %d)',p));
axis([0,1, -0.4,1.1]);

%% MSE and ME
% Residual:
res = A_final*c_final - y;
