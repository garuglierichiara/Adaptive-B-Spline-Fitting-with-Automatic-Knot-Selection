...
function [UU] = AdjustKnotsPosition_closed(delta, tol, U_tilde, p, x, y, a, b)
% INPUTS:
% - delta: knot spacing
...
while (i < length(UU))
    ...
    Ed = LeastSquareFit_closed(Mdt, p, x, y, a, b);    % Compute the LS error

    % Mpt: inserting a double knot at the midpoint (Split, increase the multiplicity)
    Mpt = [UU(1:i), mid, mid, UU(i+1:end)];
    Ep = LeastSquareFit_closed(Mpt, p, x, y, a ,b);    % Compute the LS error
    ...
    while (temp_next - temp_pre > tol)      % Stopping criterion: we stop the bisection when the interval is small enough
        ...
        Lerr = LeastSquareFit_closed(Lt, p, x, y, a ,b);   % Compute the error

        % Try to move the right boundary knot  
        Rt = [UU(1:i-1), mid_point, UU(i+1:end)];
        Rerr = LeastSquareFit_closed(Rt, p, x, y, a ,b);   % Compute the error
